...
public class NetFrontBrowserBuilder extends LayoutEngineBrowserBuilder {

    private static final String VERSION_REGEXP = ".*?(?:(?:Net[Ff]ront)|(?:ACS-NF)|(?:NF-Browser))[/ ]?[/ ]?(?:WAP)?([0-9\\.]+).*?";
    private Pattern versionPattern = Pattern.compile(VERSION_REGEXP);

    public boolean canBuild(UserAgent userAgent) {
        if (userAgent.getCompleteUserAgent().matches("(?i).*netfront.*") || userAgent.getCompleteUserAgent().contains("ACS-NF") || userAgent.getCompleteUserAgent().contains("NF-Browser")) {
            return true;
        }
        return false;
    }
