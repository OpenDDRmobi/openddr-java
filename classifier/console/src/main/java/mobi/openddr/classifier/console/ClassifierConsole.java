...
public class ClassifierConsole {
	...
	System.out.println(APP_NAME
		+ " "
		+ ClassifierConsole.class.getPackage()
	...
	lvOptions.addOption(Option.builder("u").longOpt("url").argName("url")
		.desc("Load OpenDDR resouces from URL or \"default\"")
		.hasArg().optionalArg(true).build());

	lvOptions.addOption(Option.builder("f").longOpt("folder")
		.argName("path")
		.desc("Load OpenDDR resouces from folder or \"default\"")
		.hasArg().optionalArg(true).build());
	...
	lvOptions
		.addOption(Option
			...
			.longOpt("jar")
			.argName("jar")
			.desc("Load OpenDDR resouces from JAR file in classpath or \"default\"")
			.hasArg().optionalArg(true).build());
	...
	try {
		...
		Logger.getLogger(Classifier.class.getName()).setLevel(
			debugLevel);
		Logger.getLogger(DDRLoader.class.getName())
			.setLevel(debugLevel);
		for (Handler h : Logger
			.getLogger(Classifier.class.getName()).getParent()
			.getHandlers()) {
	    ...
	    final Classifier client = new Classifier();
		...
		if (lvCmd.hasOption('o')) {
		    System.out
			    .println("OpenDDR JSON => " + device.toString());
		} else {
