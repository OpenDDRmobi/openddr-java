...
abstract class DeviceAdjuster {
    private static final Logger LOG = Logger.getLogger(DeviceAdjuster.class
	    .getName());
    ...
    static final DeviceType adjustFromUserAgent(final DeviceType device,
	...
	if (pattern != null && pattern.contains(";")) {
		...
		if (part != null) {
			...
			if (!versionPart.equals(versionExisting)) {
			    LOG.fine("Adjusting '" + versionExisting + "' to '"
				    + versionPart + "'");
			...
			if (versionCandidate.contains("OS")) {
			    ...
			    final String versionExisting = attributes
				    .get(DEVICE_OS_VERSION);
			    if (!versionPart.equals(versionExisting)) {
				LOG.fine("Adjusting '" + versionExisting
					+ "' to '" + versionPart + "'");
				attributes.put(DEVICE_OS_VERSION, versionPart);
				device.setAttributes(attributes);
			    }
			...
			if (version != null) {
			    ...
			    LOG.fine("Adjusting '" + osExisting + "' to '"
